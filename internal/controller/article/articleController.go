...
import (
	...
	"fmt"
	"net/http"
	articledto "news-release/internal/dto/article"
...
func (a *ArticleController) ListArticle(ctx *gin.Context) {
	// 初始化参数结构体并绑定查询参数
	var req articledto.ArticleListRequest
	...
	var result []articledto.ArticleListResponse
	for _, a := range article {
		result = append(result, articledto.ArticleListResponse{
...
func (p *ArticleController) GetArticleContent(ctx *gin.Context) {
	// 初始化参数结构体并绑定查询参数
	var req articledto.ArticleContentRequest
	...
	result := articledto.ArticleContentResponse{
