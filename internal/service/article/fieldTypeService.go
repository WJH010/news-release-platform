package article

import (
	"context"
	articlemodel "news-release/internal/model/article"
	articlerepo "news-release/internal/repository/article"
)

// 服务接口，定义方法，接收 context.Context 和数据模型。
type FieldTypeService interface {
	GetFieldType(ctx context.Context) ([]*articlemodel.FieldType, error)
}

// 实现接口的具体结构体，持有数据访问层接口 Repository 的实例
type FieldTypeServiceImpl struct {
	fieldTypeRepo articlerepo.FieldTypeRepository
}

// 创建服务实例
func NewFieldTypeService(fieldTypeRepo articlerepo.FieldTypeRepository) FieldTypeService {
	return &FieldTypeServiceImpl{fieldTypeRepo: fieldTypeRepo}
}

// 获取领域类型列表
func (s *FieldTypeServiceImpl) GetFieldType(ctx context.Context) ([]*articlemodel.FieldType, error) {
	return s.fieldTypeRepo.GetFieldType(ctx)
}
